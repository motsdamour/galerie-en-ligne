        ...
        {filters.map(f => (
          <button
            ...
            style={{
              ...
              background: filter === f.key ? '#2C2C2C' : 'white',
              color: filter === f.key ? 'white' : '#6B6B6B',
              border: `1px solid ${filter === f.key ? '#2C2C2C' : '#E8E4DF'}`,
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center', justifyContent: 'flex-end' }}>
                  <button
                    ...
                    style={{ background: 'transparent', border: '1px solid #E8E4DF', borderRadius: 8, padding: '5px 10px', fontSize: 11, fontFamily: "'Inter', sans-serif", cursor: 'pointer', color: '#6B6B6B', fontWeight: 500 }}
                  >
                    Email
                  </button>
                  <button
                  ...
                  <button
                    ...
                    style={{ background: 'transparent', border: 'none', fontSize: 11, fontFamily: "'Inter', sans-serif", cursor: 'pointer', color: '#b71c1c', opacity: deletingId === ev.id ? 0.5 : 1, padding: '5px 4px', fontWeight: 500 }}
                  >
                ...
                {/* Arrow */}
                <a href={`/admin/galleries/${ev.slug}`} onClick={e => e.stopPropagation()} style={{ color: '#9B9B9B', fontSize: 14, textDecoration: 'none' }}>→</a>
              </div>
            )
          })
        )}
      ...
      {qrModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 40, textAlign: 'center', maxWidth: 400, width: '100%', position: 'relative' }}>
            <button onClick={() => setQrModal(null)} style={{ position: 'absolute', top: 12, right: 16, background: 'transparent', border: 'none', fontSize: 20, cursor: 'pointer', color: '#9B9B9B' }}>×</button>
            <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontStyle: 'italic', marginBottom: 20 }}>{qrModal.name}</h3>
            ...
            <button onClick={() => downloadQR(qrModal.id)} style={{ background: '#2C2C2C', color: 'white', border: 'none', borderRadius: 10, padding: '10px 28px', fontSize: 13, fontFamily: "'Inter', sans-serif", fontWeight: 500, cursor: 'pointer' }}>Télécharger</button>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 14, padding: 32, width: '100%', maxWidth: 400 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => setEmailModal(null)} style={{ background: 'transparent', border: '1px solid #E8E4DF', borderRadius: 10, padding: '10px 16px', fontSize: 13, fontFamily: "'Inter', sans-serif", cursor: 'pointer', color: '#6B6B6B' }}>Annuler</button>
            </div>
          </div>
        </div>
      )}
